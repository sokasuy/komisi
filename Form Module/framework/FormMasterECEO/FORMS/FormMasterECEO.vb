Public Class FormMasterECEO
    ...
    Private Sub FormMasterECEO_FormClosed(sender As Object, e As FormClosedEventArgs) Handles Me.FormClosed
        Call myCFormManipulation.CloseMyForm(Me.Owner, Me)
    End Sub

    Private Sub FormMasterECEO_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            isNew = True
            ...
            stSQL = "SELECT column_name FROM INFORMATION_SCHEMA. COLUMNS WHERE TABLE_NAME = 'mseceo' and column_name NOT IN('created_at','updated_at') ORDER BY column_name ASC;"
            ...
            tableName(0) = CONN_.schemaKomisi & ".mseceo"

            isDataPrepared = True
        Catch ex As Exception
            Call myCShowMessage.ShowErrMsg("Pesan Error: " & ex.Message, "FormMasterECEO_Load Error")
        Finally
    ...
    Private Sub FormMasterECEO_Activated(sender As Object, e As EventArgs) Handles MyBase.Activated
        Try
    ...
    Private Sub FormMasterECEO_KeyDown(sender As Object, e As KeyEventArgs) Handles cboPeriode.KeyDown, cboSales.KeyDown, tbEC.KeyDown, tbEO.KeyDown, tbX.KeyDown, tbY.KeyDown, tbAkhir.KeyDown, btnSimpan.KeyDown, btnKeluar.KeyDown, btnAddNew.KeyDown, tbCari.KeyDown, btnTampilkan.KeyDown
        Try
            If (e.KeyCode = Keys.Enter) Then
                Me.SelectNextControl(Me.ActiveControl, True, True, True, True)
                If (sender Is tbY) Then
            ...
            End If
        Catch ex As Exception
            Call myCShowMessage.ShowErrMsg("Pesan Error: " & ex.Message, "FormMasterECEO_KeyDown Error")
        End Try
    End Sub
    ...
    Private Sub SetDGV(myConn As Object, myComm As Object, myReader As Object, offSet As Integer, ByRef myDataTable As DataTable, ByRef myBindingTable As BindingSource, mKriteria As String, Optional gantiKriteria As Boolean = False, Optional sortingCols As String = Nothing, Optional sortingType As String = Nothing)
        Try
            ...
            stSQL = "SELECT rid,kodesales as kode_sales,namasales as nama_sales,periode,ec,eo,x,y,akhir,created_at,updated_at " &
                    "FROM ( " &
                        "SELECT sub.rid,sub.kodesales,sub.namasales,sub.periode,sub.ec,sub.eo,sub.x,sub.y,sub.akhir,sub.created_at,sub.updated_at " &
                        "FROM ( " &
                            "SELECT tbl.rid,tbl.kodesales,tbl.namasales,tbl.periode,tbl.ec,tbl.eo,tbl.x,tbl.y,tbl.akhir,tbl.created_at,tbl.updated_at " &
            ...
            With dgvView
                ...
                .Columns("ec").DefaultCellStyle.Format = "#,##0.00"
        ...
        Try
            If (dgvView.RowCount > 0) Then
                ...
                If (e.ColumnIndex = dgvView.Columns("delete").Index) Then
                    ...
                    Dim isConfirm = myCShowMessage.GetUserResponse("Apakah mau menghapus data di master EC EO " & dgvView.CurrentRow.Cells("nama_sales").Value & " periode " & dgvView.CurrentRow.Cells("periode").Value & "?" & ControlChars.NewLine & "Data yang sudah dihapus tidak dapat dikembalikan lagi!")
                    If (isConfirm = DialogResult.Yes) Then
                        ...
                        Call myCShowMessage.ShowDeletedMsg("Data di master EC EO " & dgvView.CurrentRow.Cells("nama_sales").Value & " periode " & dgvView.CurrentRow.Cells("periode").Value)
                        Call SetDGV(CONN_.dbMain, CONN_.comm, CONN_.reader, 10, myDataTableDGV, myBindingTableDGV, mCari, True, IIf(cboSortingCriteria.SelectedIndex = -1, Nothing, cboSortingCriteria.SelectedValue), cboSortingType.SelectedItem)
                    Else
                        Call myCShowMessage.ShowInfo("Penghapusan data di master EC EO " & dgvView.CurrentRow.Cells("nama_sales").Value & " periode " & dgvView.CurrentRow.Cells("periode").Value & " dibatalkan oleh user")
                    End If
                    ...
                    If Not IsDBNull(dgvView.CurrentRow.Cells("periode").Value) Then
                        For i As Integer = 0 To cboPeriode.Items.Count - 1
                            If (DirectCast(cboPeriode.Items(i), DataRowView).Item("keterangan") = dgvView.CurrentRow.Cells("periode").Value) Then
                                ...
                                arrDefValues(1) = dgvView.CurrentRow.Cells("periode").Value
                    ...
                    If Not IsDBNull(dgvView.CurrentRow.Cells("kode_sales").Value) Then
                        For i As Integer = 0 To cboSales.Items.Count - 1
                            If (DirectCast(cboSales.Items(i), DataRowView).Item("kodesales") = dgvView.CurrentRow.Cells("kode_sales").Value) Then
                                ...
                                arrDefValues(2) = dgvView.CurrentRow.Cells("kode_sales").Value
                                arrDefValues(3) = dgvView.CurrentRow.Cells("nama_sales").Value
                                cboSales.Enabled = False
                            End If
                        Next
                    End If
                    'EC
                    If Not IsDBNull(dgvView.CurrentRow.Cells("ec").Value) Then
                        tbEC.Text = dgvView.CurrentRow.Cells("ec").Value
                        arrDefValues(4) = dgvView.CurrentRow.Cells("ec").Value
                    End If
                    'EO
                    If Not IsDBNull(dgvView.CurrentRow.Cells("eo").Value) Then
                        tbEO.Text = dgvView.CurrentRow.Cells("eo").Value
                        arrDefValues(5) = dgvView.CurrentRow.Cells("eo").Value
                    End If
                    'X
                    If Not IsDBNull(dgvView.CurrentRow.Cells("x").Value) Then
                        tbX.Text = dgvView.CurrentRow.Cells("x").Value
                        arrDefValues(6) = dgvView.CurrentRow.Cells("x").Value
                    End If
                    'Y
                    If Not IsDBNull(dgvView.CurrentRow.Cells("y").Value) Then
                        tbY.Text = dgvView.CurrentRow.Cells("y").Value
                        arrDefValues(7) = dgvView.CurrentRow.Cells("y").Value
                    End If
                    'Akhir
                    If Not IsDBNull(dgvView.CurrentRow.Cells("akhir").Value) Then
                        tbAkhir.Text = dgvView.CurrentRow.Cells("akhir").Value
                        arrDefValues(8) = dgvView.CurrentRow.Cells("akhir").Value
                    End If
                    isDataPrepared = True
                End If
            End If
    ...
    Private Sub btnSimpan_Click(sender As Object, e As EventArgs) Handles btnSimpan.Click
        Try
            If (cboPeriode.SelectedIndex <> -1 And cboSales.SelectedIndex <> -1 And Trim(tbEC.Text).Length > 0 And Trim(tbEO.Text).Length > 0 And Trim(tbX.Text).Length > 0 And Trim(tbY.Text).Length > 0) Then
                Me.Cursor = Cursors.WaitCursor
                Call myCDBConnection.OpenConn(CONN_.dbMain)
                ...
                If isNew Then
                    'CREATE NEW
                    isExist = myCDBOperation.IsExistRecords(CONN_.dbMain, CONN_.comm, CONN_.reader, "rid", tableName(0), "periode='" & myCStringManipulation.SafeSqlLiteral(cboPeriode.SelectedValue) & "' AND kodesales='" & myCStringManipulation.SafeSqlLiteral(cboSales.SelectedValue) & "'")
                    If Not isExist Then
                        'CREATE NEW
                        newValues = "'" & myCStringManipulation.SafeSqlLiteral(cboPeriode.SelectedValue) & "','" & myCStringManipulation.SafeSqlLiteral(cboSales.SelectedValue) & "','" & myCStringManipulation.SafeSqlLiteral(DirectCast(cboSales.SelectedItem, DataRowView).Item("namasales")) & "'," & Double.Parse(tbEC.Text) & "," & Double.Parse(tbEO.Text) & "," & Double.Parse(tbX.Text) & "," & Double.Parse(tbY.Text) & "," & ADD_INFO_.newValues
                        newFields = "periode,kodesales,namasales,ec,eo,x,y," & ADD_INFO_.newFields
                        Call myCDBOperation.InsertData(CONN_.dbMain, CONN_.comm, tableName(0), newValues, newFields)
                        Call myCShowMessage.ShowSavedMsg("Data di master EC EO " & DirectCast(cboSales.SelectedItem, DataRowView).Item("namasales") & " periode " & cboPeriode.SelectedValue & " berhasil dimasukkan")
                        Call btnTampilkan_Click(sender, e)

                        Call myCFormManipulation.ResetForm(gbDataEntry)
                        Call btnCreateNew_Click(sender, e)
                    Else
                        Call myCShowMessage.ShowWarning("Sudah ada data di master EC EO " & DirectCast(cboSales.SelectedItem, DataRowView).Item("namasales") & " periode " & cboPeriode.SelectedValue & " !!")
                    End If
                Else
                    'UDPATE
                    ...
                    If (arrDefValues(1) <> Trim(cboPeriode.SelectedValue)) Then
                        isExist = myCDBOperation.IsExistRecords(CONN_.dbMain, CONN_.comm, CONN_.reader, "rid", tableName(0), "kodesales='" & myCStringManipulation.SafeSqlLiteral(cboSales.SelectedValue) & "' and periode='" & cboPeriode.SelectedValue & "'")
                        If Not isExist Then
                            updateString &= IIf(IsNothing(updateString), "", ",") & "periode=" & IIf(Trim(cboPeriode.SelectedValue).Length = 0, "Null", "'" & myCStringManipulation.SafeSqlLiteral(cboPeriode.SelectedValue) & "'")
                            If (foundRows.Length > 0) Then
                                myDataTableDGV.Rows(myDataTableDGV.Rows.IndexOf(foundRows(0))).Item("periode") = Trim(cboPeriode.SelectedValue)
                            End If
                        Else
                            Call myCShowMessage.ShowWarning("Sales " & DirectCast(cboSales.SelectedItem, DataRowView).Item("namasales") & " untuk periode " & cboPeriode.SelectedValue & " sudah terdaftar" & ControlChars.NewLine & "EC EO tidak boleh ada dobel!!")
                        End If
                    End If
                    If (arrDefValues(2) <> Trim(cboSales.SelectedValue)) Then
                        isExist = myCDBOperation.IsExistRecords(CONN_.dbMain, CONN_.comm, CONN_.reader, "rid", tableName(0), "kodesales='" & myCStringManipulation.SafeSqlLiteral(cboSales.SelectedValue) & "' and periode='" & cboPeriode.SelectedValue & "'")
                        If Not isExist Then
                            updateString &= IIf(IsNothing(updateString), "", ",") & "kodesales='" & myCStringManipulation.SafeSqlLiteral(cboSales.SelectedValue) & "',namasales='" & myCStringManipulation.SafeSqlLiteral(DirectCast(cboSales.SelectedItem, DataRowView).Item("namasales")) & "'"
                            If (foundRows.Length > 0) Then
                    ...
                    If Not IsNothing(updateString) Then
                        ...
                        Call myCShowMessage.ShowUpdatedMsg("Data di master EC EO " & DirectCast(cboSales.SelectedItem, DataRowView).Item("namasales") & " periode " & cboPeriode.SelectedValue)
    ...
    Private Sub tbNumeric_Validated(sender As Object, e As EventArgs) Handles tbEC.Validated, tbEO.Validated, tbX.Validated, tbY.Validated, tbAkhir.Validated
        ...
        End Try
    End Sub
    Private Sub cboFields_Validated(sender As Object, e As EventArgs) Handles cboPeriode.Validated, cboSales.Validated
        Try
